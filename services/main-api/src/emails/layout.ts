...
function renderBlockHtml(b: Block): string {
  switch (b.type) {
    ...
    case 'heading':
      return `<h1 style="margin:0 0 20px 0;font-family:${TOKENS.serif};font-style:italic;font-weight:500;font-size:34px;line-height:1.2;color:${TOKENS.textPrimary};text-align:center;">${esc(b.text)}</h1>`;
    ...
    case 'button':
      // Solid dark button, square-ish corners. Inline a wrapper so spacing
      // around the button works even in clients that strip <p> margins.
      return `<div style="text-align:center;margin:28px 0;"><a href="${esc(b.href)}" style="display:inline-block;padding:14px 36px;background:${TOKENS.buttonBg};color:${TOKENS.buttonText};text-decoration:none;font-family:${TOKENS.sans};font-size:14px;font-weight:600;letter-spacing:0.04em;border-radius:4px;">${esc(b.label)}</a></div>`;

    case 'linkText':
      return `<p style="margin:0 0 16px 0;font-family:${TOKENS.sans};font-size:13px;color:${TOKENS.textMuted};text-align:center;word-break:break-all;"><a href="${esc(b.href)}" style="color:${TOKENS.textPrimary};text-decoration:underline;">${esc(b.label)}</a></p>`;

    case 'small':
      return `<p style="margin:18px 0 0 0;font-family:${TOKENS.sans};font-size:12px;line-height:1.55;color:${TOKENS.textFaint};text-align:center;">${esc(b.text)}</p>`;
    ...
    case 'divider':
      return `<div style="margin:28px auto;width:60px;height:1px;background:${TOKENS.accent};"></div>`;

    case 'list': {
      const items = b.items.map(item => {
        return `<li style="padding:6px 0 6px 22px;position:relative;font-family:${TOKENS.sans};font-size:14px;line-height:1.55;color:${TOKENS.textBody};"><span style="position:absolute;left:0;top:6px;color:${TOKENS.accent};font-weight:700;">·</span>${esc(item)}</li>`;
      }).join('');
...
export interface LayoutOptions {
  ...
  footer: string;
  /** Localized "view in browser" / unsubscribe-ish link text — currently unused, reserved. */
  blocks: Block[];
}
...
<div style="text-align:center;padding:24px 16px 8px 16px;">
  <p style="margin:0 0 6px 0;font-family:${TOKENS.sans};font-size:12px;line-height:1.55;color:${TOKENS.textMuted};white-space:pre-line;">${esc(opts.footer)}</p>
  <p style="margin:8px 0 0 0;font-family:${TOKENS.sans};font-size:11px;color:${TOKENS.textFaint};">© ${year} Evenero · <a href="https://evenero.com" style="color:${TOKENS.textFaint};text-decoration:none;">evenero.com</a></p>
</div>
